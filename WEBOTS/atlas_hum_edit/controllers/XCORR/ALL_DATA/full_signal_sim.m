...
per_sim=abs((xc_OL/FULL)*100);
sig_sim(x,i) = per_sim;
end
avg_sig_sim(x,1)= mean(sig_sim(x,:));
end
[max_v,ind] = max(avg_sig_sim);
%%%%%%%%%%%%% SHOW RESULT
FORCE_RES = FORCE(ind);
POSITION_RES = POSITION(ind);
save ('sig_sim');
save ('avg_sig_sim');
xlswrite('sig_sim', sig_sim)
xlswrite('avg_sig_sim', avg_sig_sim)
%DATASET.xlsx is created manually
xcor_DATADET= readtable('xcor_DATASET.xlsx');
